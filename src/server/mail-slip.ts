...
export function slipHtml(input: {
	kicker: string;
	heading: string;
	body: string;
	...
	const codeBlock = input.code?.trim()
		? `<p style="margin:26px 0 0;padding-top:18px;border-top:1px dashed #d7c4ae;font-family:Courier,monospace;font-size:12px;letter-spacing:0.08em;color:#7a6554;">
									...
									<span style="display:inline-block;margin-top:6px;font-size:24px;letter-spacing:0.32em;color:#2c1d16;">${escapeHtml(input.code.trim())}</span>
								</p>`
		: '';
