...
export default async function DepartmentPage({ params }: { params: Promise<{ slug: string }> }) {
	...
	return (
		<article className="min-w-0">
			<p className="font-mono text-xs tracking-[0.16em] text-muted-foreground uppercase">{current.code}</p>
			<div className="mt-2 flex min-w-0 flex-wrap items-end justify-between gap-3">
				...
				<p className="font-body text-[0.95rem] text-muted-foreground italic">{booksLabel(books.length)}</p>
			</div>
